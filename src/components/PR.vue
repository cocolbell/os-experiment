...
export default {
    ...
    methods : {
        ...
        excute () {
            ...
            this.runPcbList.push(this.waitPcbList.shift());
            ...
            var processor = setInterval (function () {
                runningPcb = vm.runPcbList[vm.runPcbList.length - 1]; 
                runningPcb.needTime--;
                runningPcb.runTime++;
                timer++;
                if(timer == vm.timeLine) { //如果时间片已经结束
                    timer = 0;
                    
                    //时间片结束后，调度程序判断进程是否结束来决定是否调度
                    if(runningPcb.needTime == 0) {  //如果此进程运行结束
                        runningPcb.status = 'finish';
                        if(vm.waitPcbList.length) {
                            vm.runPcbList.push(vm.waitPcbList.shift());  
                        ...
                        runningPcb.status = 'wait';                        
                        vm.waitPcbList.push(vm.runPcbList.pop());
                        vm.runPcbList.push(vm.waitPcbList.shift());
                    }                    
                }
                //时间片未结束，调度程序判断进程是否结束来决定是否调度
                else if(runningPcb.needTime == 0) {  //如果此进程运行结束
                    runningPcb.status = 'finish';
                    if(vm.waitPcbList.length) {
                        vm.runPcbList.push(vm.waitPcbList.shift());  
                        vm.runPcbList[vm.runPcbList.length - 1].status = 'run'
